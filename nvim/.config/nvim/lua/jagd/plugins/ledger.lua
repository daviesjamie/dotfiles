return {
    ...
    config = function()
        ...
        -- Remap `ga` and `<leader>f` bindings to use vim-ledger's formatting
        vim.keymap.set("v", "ga", ":LedgerAlign<CR>", { desc = "Align" })
        vim.keymap.set("n", "<leader>f", ":LedgerAlignBuffer<CR>", { desc = "[F]ormat buffer" })
        vim.keymap.set("v", "<leader>f", ":LedgerAlign<CR>", { desc = "[F]ormat selection" })
        ...
        vim.keymap.set(
            ...
            { desc = "Toggle transaction [S]tatus" }
        )
    end,
}
